...
import (
	...
	pchainapi "github.com/DioneProtocol/odysseygo/vms/omegavm/api"
...
var (
	...
	// Omega address resulting from the above private key
	testAddress = "O-testing18jma8ppw3nhx5r4ap8clazz0dps7rv5umpc36y"
...
func TestGetTxStatus(t *testing.T) {
	...
	peerSharedMemory := m.NewSharedMemory(aChainID)
	...
	tx, err := service.vm.txBuilder.NewImportTx(aChainID, ids.ShortEmpty, []*secp256k1.PrivateKey{recipientKey}, ids.ShortEmpty)
	require.NoError(err)
		...
		},
		{
			...
			func(service *Service) (*txs.Tx, error) {
				return service.vm.txBuilder.NewAddValidatorTx( // Test GetTx works for proposal blocks
					...
					uint64(service.vm.clock.Time().Add(txexecutor.SyncBound).Add(defaultMinValidatorStakingDuration).Unix()),
...
func TestGetStake(t *testing.T) {
	...
	for i, validator := range genesis.Validators {
		addr := fmt.Sprintf("O-%s", validator.RewardOwner.Addresses[0])
	...
	delegatorEndTime := uint64(defaultGenesisTime.Add(defaultMinDelegatorStakingDuration).Unix())
	...
	pendingStakerEndTime := uint64(defaultGenesisTime.Add(defaultMinValidatorStakingDuration).Unix())
