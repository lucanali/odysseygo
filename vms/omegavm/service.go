...
import (
	...
	omegaapi "github.com/DioneProtocol/odysseygo/vms/omegavm/api"
)
...
func (s *Service) GetCurrentValidators(_ *http.Request, args *GetCurrentValidatorsArgs, reply *GetCurrentValidatorsReply) error {
	...
	for _, currentStaker := range targetStakers {
		...
		case txs.PrimaryNetworkValidatorCurrentPriority, txs.SubnetPermissionlessValidatorCurrentPriority:
			...
			var (
				validationRewardOwner *omegaapi.Owner
				delegationRewardOwner *omegaapi.Owner
			)
			...
			vdr := omegaapi.PermissionlessValidator{
				Staker:                 apiStaker,
				Uptime:                 uptime,
				Connected:              connected,
		...
		case txs.SubnetPermissionedValidatorCurrentPriority:
			...
			connected := s.vm.uptimeManager.IsConnected(nodeID, args.SubnetID)
			reply.Validators = append(reply.Validators, omegaapi.PermissionedValidator{
				Staker:    apiStaker,
				Connected: connected,
				Uptime:    uptime,
			})
	...
	for i, vdrIntf := range reply.Validators {
		vdr, ok := vdrIntf.(omegaapi.PermissionlessValidator)
		if !ok {
			continue
		}
...
func (s *Service) getAPIOwner(owner *secp256k1fx.OutputOwners) (*omegaapi.Owner, error) {
	apiOwner := &omegaapi.Owner{
